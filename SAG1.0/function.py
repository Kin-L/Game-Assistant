...
colorzone = {"red":[156,191,43,255,46,255],
...
class imitate:
    ...
    def shotzone(self,zone):
        ...
        sc.save(scpic)
        return scpic
    def findpic(self,bpic, picpath ,size_zoom=0,method =cv2.TM_CCOEFF_NORMED,rm=False):
        if isinstance(bpic,str):
            ...
            # bp = cv2.resize(sc,)
        else:print("findpic_error:不支持的大图格式。")
        ...
        if (min_val>=-0.6) and (max_val<=0.6):pos,val=((0,0),0)
            ...
            y = y1 + sly + int(lpsy / 2)
            pos = (x, y)
        if isinstance(bpic,tuple) or rm:os.remove(scpath)
        return (pos,val)


    def findcolor(self,bpic, colorlist,dif=0):
        if isinstance(bpic, str):
            scpath = bpic
            bp = cv2.imread(bpic)
            x1, y1, x2, y2 = 0,0,0,0
            ...
            scpath = imitate.screenshot(self)
            sc = cv2.imread(scpath)
            bp = cv2.imread(scpath)[y21:y22,x21:x22]
        ...
        for color in colorlist.split("+"):
            hfg,sfg,vfg =  0,0,0
            if color in ["red","orange","yellow","green","cyan","blue","purple","white","black","grey"]:
                lh,bh, ls, bs,lv,bv = colorzone[color]
            else:
                cb, cg, cr =  tuple(int(color[i:i + 2], 16) for i in (0, 2, 4))
                H,S ,V = colorsys.rgb_to_hsv(cr, cg, cb)
                if H<0.061:H+=1
                lh,bh, ls, bs,lv,bv = H*180-10,H*180+10,S*255-51,S*255+51,V-51,V+51
            for x in range(bpw):
                for y in range(bph):
                    h,s,v = HSV[y,x]
...
def start(time,path):
    if os.path.isfile(path):
        ...
        os.system("start /d \""+dir+"\" "+file+" -popupwindow")
        wait(time)
        return False
    else:
        print("设定的路径不正确："+path)
