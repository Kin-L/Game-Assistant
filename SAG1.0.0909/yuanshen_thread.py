...
class Thready(QThread):
    ...
    def run(self):
        ...
        if self.tlist[1][8]:
            ...
            if killgame(self.tlist[0][0], "UnityWndClass", "原神"):
    ...
    def yuanshen_start(self):
        imi = imitate("UnityWndClass", "原神",8000,self.tlist[0][0])
        print(self.tlist[0])
        ...
        while 1:
            scpath = shotzone()
            if serverflag ==0:
                if findpic( r"genshin_resource\picture\login1.png",(900, 995, 1030, 1043),scpath)[1] >= 0.6:
                    serverflag =2
                    click(930, 630)
                ...
                if findpic( r"genshin_resource\picture\login2.png",(863, 370, 1059, 467),scpath)[1] >= 0.6:
                    logflag = False
                    click(953,659)
                ...
                if findpic( r"genshin_resource\picture\login1.png",(900, 995, 1030, 1043),scpath)[1] >= 0.6:
                    ...
                    os.remove(scpath)
                    scpath = shotzone()
            if findpic( r"genshin_resource\picture\sighin.png",(865, 240, 1060, 470),scpath)[1] >= 0.6:
                sighinflag = False
                click(930, 850)
                wait(800)
                click(930, 850)
                ...
                os.remove(scpath)
                scpath = shotzone()
            if findpic( r"genshin_resource\picture\world.png",(57, 998, 179, 1075),scpath)[1] >= 0.6:
                self.testsignal.emit("加载到世界。")
                os.remove(scpath)
                break
            if findpic( r"genshin_resource\picture\home\home.png",(0, 0, 97, 88),scpath)[1] >= 0.6:
    ...
    def team(self):
        ...
        for i in range(15):
            wait(1000)
            res = findpic( r"genshin_resource\picture\team.png",(37,980, 115,1058))
            if res[1] >= 0.6:
    ...
    def world(self):
        ...
        while i >0 :
            i+=1
            wait(1000)
            pos,val = findpic( r"genshin_resource\picture\world.png",(57, 998, 179, 1075))
            if val >=0.6:
                i=0
                self.testsignal.emit("加载到世界。")
            elif i == 15:
    ...
    def tpxm(self):
        self.home()
        self.testsignal.emit("前往须弥：须弥城锚点1")
        self.opensub("地图")
        self.map_region("须弥")
        click(1008,599)
        wait(800)
        self.tp_point()
        self.testsignal.emit("到达须弥：须弥城锚点1")
    ...
    def dispatch(self):
        ...
        click(960, 900)
        wait(1500)
        (x,y), val = findpic( r"genshin_resource\picture\dispatch\dispatch.png",(1247,311, 1337,909 ))
        if (x,y) ==(0,0):
        ...
        if res:
            ...
            for g in range(len(glist)):#领取
                ...
                while num ==-1:
                    ...
                    wait(800)
                    pos, val = findpic( r"genshin_resource\picture\dispatch\get.png",(1490, 979, 1823, 1045))
                    if val>=0.6:
                        click(1692, 1024)
                        wait(1500)
                        click(1692, 1024)
        ...
        for i in range(6):
            val = findpic( r'genshin_resource\picture\number\%s.png' % (i),scpath=scpath)[1]
        ...
        if num != 5:
            ...
            for num in range(5):#派遣
                ...
                if findpic(r"genshin_resource\picture\dispatch\recall.png",scpath = scpath)[1] >=0.6:
                    self.testsignal.emit("执行中："+cname)
                    os.remove(scpath)
                elif findpic( r"genshin_resource\picture\dispatch\max.png",scpath = scpath)[1] >=0.6:
                    self.testsignal.emit("已达派遣上限")
                    os.remove(scpath)
                    break
                else:
                    res1 = findpic( r"genshin_resource\picture\dispatch\choose.png",scpath = scpath)[1]
                    res2 = findpic( r"genshin_resource\picture\dispatch\get.png",scpath = scpath,deleteflag=True)[1]
                    if res2>=0.7:
                        click(1692, 1024)
                        wait(1500)
                        click(1692, 1024)
                    ...
                    if res1>=0.7 or res2>=0.7:
                        ...
                        for y in ylist:
                            ...
                            if findpic(r"genshin_resource\picture\dispatch\exploring.png",scpath=scpath)[1]>=0.75:
                                os.remove(scpath)
                                pass
                            else:
                                res1 = findpic(r"genshin_resource\picture\dispatch\20h.png",scpath=scpath)[1]
                                res2 = findpic(r"genshin_resource\picture\dispatch\25percent.png",scpath=scpath)[1]
                                res3 = findpic(r"genshin_resource\picture\dispatch\no_promote.png",scpath = scpath,deleteflag=True)[1]
    ...
    def home(self):
        ...
        while m >= 0:
            m += 1
            wait(1500)
            (x, y), val = findpic( r"genshin_resource\picture\home\home.png",(0, 0, 97, 88))
            if val >= 0.6:m = -1
            else:press("esc")
            if m == 15:
    ...
    def map_region(self,str):
        tdir = {"蒙德":"mondstadt" ,"璃月":"liyue" ,"稻妻":"inazuma" ,"须弥":"sumeru" ,"地下矿区":"underground" }
        click(1835, 1024)
        wait(800)
        roll(1657, 747, 5)
        wait(800)
        (x1, y1), val1 = findpic( "genshin_resource\picture\maps\\"+tdir[str]+".png",(1445, 83, 1608, 1012))
        (x2, y2), val2 = findpic( r"genshin_resource\picture\maps\now.png",(1445, 83, 1608, 1012))
    ...
    def tp_point(self,num=0):
        (x, y), val = findpic( "genshin_resource\picture\maps\\tp_point%s.png"%(num),(1245, 621, 1528, 1023))
        if val >= 0.75:
            click(x, y)
            wait(800)
        click(1634, 1003)
    ...
    def tply(self):
        self.home()
        self.testsignal.emit("前往璃月：璃月港锚点1")
        self.opensub("map")
        self.map_region("璃月")
        click(958,537 )
        wait(800)
        self.tp_point()
        self.testsignal.emit("到达璃月：璃月港锚点1")
    ...
    def enter_rambler(self):
        ...
        click(1053, 48)
        wait(800)
        (x,y),val=findpic(r"genshin_resource\picture\lit_tools\rambler.png",(109,113,1275,459))
        click(x,y)
        wait(800)
        click(1694, 1022)
        wait(800)
    ...
    def make_condensed(self):
        ...
        click(960, 950)
        wait(1500)
        (x,y),val = findpic(r"genshin_resource\picture\valu_tools\nssz.png",(57,110,627,800))
        if val >= 0.6:
            click(x,y)
            wait(800)
            click(1298,400)
            ...
            if num != None:
                ...
                self.accomplish.emit(3)
        else:self.testsignal.emit("无法合成浓缩树脂：缺少树脂或晶核")
        click(1836,48)
        wait(2200)
    ...
    def tp_domain(self,str):
        ...
        for num in range(15):
            wait(200)
            (x, y), val = findpic( r"genshin_resource\picture\domain\\"+tdir[str]+".png",(752, 270, 977, 886))
            if val >=0.8:
                click(1555, y)
                wait(2000)
                break
    ...
    def use_transformer(self):
        ...
        click(1053, 48)
        wait(800)
        (x, y), val = findpic( r"genshin_resource\picture\lit_tools\para_trans.png",(109, 113, 1275, 459))
        if val >= 0.75:
            self.testsignal.emit("参量质变仪可用")
            click(x, y)
            wait(800)
            (x, y), val = findpic( r"genshin_resource\picture\lit_tools\para_retrieve.png",(1645, 971, 1769, 1058))
            if val>=0.6:
                click(x, y)
                wait(1500)
            else:
            ...
            for str in self.tlist[4]:
                ...
                (x, y), val = findpic( "genshin_resource\picture\\"+a+"\\"+a+".png",(456,0,1450,94))
                click(x, y)
                wait(800)
                ...
                click(497,1021)
                wait(800)
                res,(cx,cy) = findcolor("D8E5EC",(1406,1001,1446,1033))
                if res:
                    click(1703,1020)
                    wait(800)
                    click(1178, 757)
                    ...
                    break
                else:self.testsignal.emit("参量质变仪还未装满。")
            self.testsignal.emit("参量质变仪充能中。")
            i=0
            while i>=0:
                i+=1
                wait(2000)
                pos, val = findpic( r"genshin_resource\picture\acquire.png",(871,242, 1050,339))
                if val>=0.6:
                    i=-1
                    click(961,804)
    ...
    def cut_tree(self):
        ...
        if not (cirnum and len(runlist) >= 5):self.testsignal.emit("未设置有效伐木计划。请保证循环次数大于等于1，并且至少勾选5个伐木位点。")
            ...
            click(1053, 48)
            wait(800)
            (x, y), val = findpic( r"genshin_resource\picture\lit_tools\boon_elder_tree.png",(110, 112, 1273, 805))
            if val < 0.75:
                ...
                click(x, y)
                wait(800)
                if findpic( r"genshin_resource\picture\lit_tools\unload.png",(1637, 972, 1765, 1068))[1] >= 0.75:
