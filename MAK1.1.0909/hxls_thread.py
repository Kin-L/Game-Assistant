...
class Thready(QThread):
    ...
    def hxls_start(self):
        ...
        while 1:
            scpath = shotzone()
            if serverflag==0:
                if findpic(r"hxls_resource\picture\startgame1.png",(871, 612, 1052, 654),scpath)[1] >= 0.6:
                    wait(300)
                    click(930, 630)
                ...
                if findpic( r"hxls_resource\picture\startgame2.png",(830, 595, 1093, 682),scpath)[1] >= 0.6:
                    wait(300)
                    click(960,633)
                ...
                if findcolor( "blue",(910, 658, 992, 704))[0]:
                    ...
                    os.remove(scpath)
                    scpath = shotzone()
            if findpic( r"hxls_resource\picture\sighin.png",(887, 240, 1032, 280),scpath)[1] >= 0.6: # 签到奖励
                click(970, 930)
                wait(1500)
                click(1789, 120)
                ...
                os.remove(scpath)
                scpath = shotzone()
            (x, y), sim = findpic(r"hxls_resource\picture\close\close2.png",scpath=scpath)
            if sim >= 0.6:
                click(x, y)
                wait(1500)
            if findpic( r"hxls_resource\picture\home.png",(1739, 37, 1814, 98),scpath)[1] >= 0.6:# 主界面
                wait(1500)
                if findpic(r"hxls_resource\picture\home.png", (1739, 37, 1814, 98), scpath)[1] >= 0.6:
    ...
    def fight(self):
        ...
        val = findpic( r"hxls_resource\picture\fight\fighting.png",(1652, 306, 1788, 393))[1]
        if val>=0.6:
            ...
            wait(1500)
            while 1:
                if findpic( r"hxls_resource\picture\fight\retour.png",(1670, 50, 1820, 110))[1] >= 0.6:
                    if self.tlist[2][0]:
                        self.testsignal.emit("尝试重复上次作战。")
                        click(1744, 80)
                        wait(1800)
                        ((x, y), sim) = findpic( r"hxls_resource\picture\fight\add.png",(1100, 335, 1900, 983))
                        if sim >= 0.6:
                            click(x + 120, y)
                            wait(1000)
                            click(x, y + 280)
                            wait(1000)
                            if findpic( r"hxls_resource\picture\fight\lack.png",(1085, 716,1256, 846))[1]>0.75:
                        ...
                        wait(800)
                    break
                if findpic( r"hxls_resource\picture\lvup.png",(875, 182, 1048, 221))[1] >= 0.6:
    ...
    def dispatch(self):#
        ...
        for n in range(6):
            ...
            click(x, y)
            wait(200)
            if findpic( r"hxls_resource\picture\dispatch\condition2.png",zoom)[1] >= 0.6:  # 完成派遣
                x, y = self.indexdir["线下采购坐标"]
                click(x, y)
                wait(500)
                click(1708, 977)
                wait(1200)
            ...
            elif findpic( r"hxls_resource\picture\dispatch\condition1.png",(1632, 927,1796, 1013))[1]>=0.6:
                self.testsignal.emit("线下采购" + str(n + 1) + "：进行中。")
            elif findpic( r"hxls_resource\picture\dispatch\condition0.png",zoom)[1]>=0.6:#派遣
                dlist += [n]
        ...
        for n in dlist:
            ...
            click(1571, 883)
            wait(1000)
            (x, y), sim = findpic( r"hxls_resource\picture\dispatch\plan"+str(pnum)+".png",(194, 145, 454, 585))
            if sim >=0.85:
                click(x, y)
                wait(500)
    ...
    def review(self):
        ...
        if findcolor( "red",(1631,876,1708,958))[0]:
            ...
            for (x1,y1,x2,y2,n) in list:
                (x,y),val = findpic( r"hxls_resource\picture\review\reviewed.png",(x1,y1,x2,y2))
                if val >= 0.6:#完成回顾
                    ...
                    (x,y),val = findpic( r"hxls_resource\picture\review\toreview.png",(x1,y1,x2,y2))
                    if val >= 0.6:#开始回顾
    ...
    def getmarket(self):
        ...
        if not findcolor( "red",(304, 254, 377, 324))[0]:
            ...
            click(300, 383)
            wait(1000)
            if findcolor("red",(294, 440, 334, 486))[0]:
                click(217,487)
                wait(500)
                if findpic( r"hxls_resource\picture\market\daily.png",(510, 250, 684, 334))[1]>=0.6:
                    click(601,247)
                    wait(1200)
                    click(1257,723)
                    wait(1200)
            ...
            if findcolor( "red",(270, 538, 355, 613))[0]:
                click(214, 598)
                wait(1000)
                click(1516, 50)
                wait(2000)
    ...
    def recruit(self):
        ...
        for n in range(3):
            lzone = tzone[n]
            x, y = tclick[n]
            if findpic( r"hxls_resource\picture\recruit\accomplish.png",lzone)[1] >= 0.6:
                self.testsignal.emit("访募%s：完成。尝试领取。"%(n+1))
                self.receive_recruit(x, y)
            while 1:
                if findpic( r"hxls_resource\picture\recruit\new.png",lzone)[1] >= 0.6:
                    ...
                    if findcolor( "2F2C30",(480, 253,499, 273))[0]:
                        ...
                        if (not cflag) or (not vflag):break
                            ...
                            click(743, 811)
                            wait(1000)
                            if findpic( r"hxls_resource\picture\recruit\lack.png",(903, 70, 1021, 141))[1]>=0.6:
    ...
    def receive_recruit(self,x,y):
        ...
        wait(2000)
        scpath = shotzone()
        if findpic( r"hxls_resource\picture\recruit\N.png",(252, 444, 421, 553),scpath)[1] >= 0.75:
            self.testsignal.emit("访募到N卡。")
        else:
            if findcolor( "blue",(277, 474, 395, 528),scpath)[0]:
                ...
                os.remove(path)
                if findcolor( "purple",(277, 474, 395, 528),scpath)[0]:
                    self.testsignal.emit("访募到SR卡,可在文件夹“hxls_resource\screenshot”中查看。")
                else:
                    if findcolor( "yellow",(277, 474, 395, 528),scpath)[0]:
